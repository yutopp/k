...
(defmacro int-op-defn (hook-name function-name)
  (int-op-fn hook-name function-name))
(defmacro int-rel-defn (hook-name relation-name)
  (int-rel-fn hook-name relation-name))
...
(include-book "syntax")
(defun primitive-syntax ()
  '(("#Int" :element integerp)
    ("#Id" :element symbolp)
    ("#String" :element stringp)
    ("#Bool" :group
     ("'true" "'false"))
    ("'true" :element nil)
    ("'false" :element nil)))
(make-event (syntax::make-constructors '|| (primitive-syntax)))
(make-event (syntax::make-predicates '|| (primitive-syntax)))
