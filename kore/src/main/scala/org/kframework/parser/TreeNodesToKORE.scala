...
object TreeNodesToKORE {
  ...
  def apply(t: Term): K = t match {
    ...
    case Ambiguity(items, l) => KApply(CrazyModule.KLabel("AMB"), KList(items.asScala.toList map apply asJava), Att())
  }
  ...
  def down(t: K): K = t match {
    ...
    case t@KApply(KLabel("#KRewrite", _), items: kore.KList) =>
      val it = items.items.iterator
      val res = KRewrite(down(it.next()), down(it.next()), t.att)
      assert(!it.hasNext)
      res

    case t@KApply(KLabel("#KApply", _), items) =>
      val theKList = items.tail.head match {
        case t@KApply(KLabel("#KList", _), items) => KList((items map down _).asJava)
        case c: KToken => KList(down(c))
      }
      KApply(
        CrazyModule.KLabel(items(0).asInstanceOf[KToken].s),
        theKList, t.att)

    case t@KApply(KLabel("#KToken", _), items) =>
